<template>
    <div id="login-view">
        <div class="login-region">
            ...
            <div class="login-input">
                <el-input placeholder="商家密码" v-model="password" clearable show-password/>
            </div>
...
<script setup>
    ...
    const mobile = ref('15820161201')
    const password = ref('a1234567')
    const loadIng = ref(false)
    async function goLogin(){
        try {
            loadIng.value = true
            const res = await request.post('/login',{phone:mobile.value,password:password.value})
